...
<body>
	...
	<div class="content">
		...
		<div class="mainbar">
			<div class="page-head">
				<h2 class="pull-left">SQL详情</h2>
				<div class="bread-crumb pull-right">
					<a href="/home/index"><i class="icon-home"></i>首页</a><span class="divider">/</span>SQL详情
				</div>
									...
									<div class="padd form-horizontal">
										<c:if test="${!Success}">
											<div class="form-group">
												<div class="col-lg-12" style="text-align: center;">SQL不存在!!!</div>
											</div>
										</c:if>
										<c:if test="${Success}">
											<div class="form-group">
												<label class="col-lg-5 control-label" >SQL名称:</label>
												<div class="col-lg-7">${ResultData.name}</div>
											</div>
											<div class="form-group">
												<label class="col-lg-5 control-label" >完整SQL:</label>
												<div class="col-lg-7">
												<textarea class="form-control" rows="3" disabled="disabled">${ResultData.dap_sql}</textarea>
												</div>
											</div>
											
											<div class="form-group">
												<label class="col-lg-5 control-label" >SELECT块:</label>
												<div class="col-lg-7">
												<textarea class="form-control" rows="3" disabled="disabled">${ResultData.sql_select_clause}</textarea>
												</div>
											</div>
											
											<div class="form-group">
												<label class="col-lg-5 control-label" >FROM块:</label>
												<div class="col-lg-7">
												<textarea class="form-control" rows="3" disabled="disabled">${ResultData.sql_from_clause}</textarea>
												</div>
											</div>
											
											<div class="form-group">
												<label class="col-lg-5 control-label" >WHERE块:</label>
												<div class="col-lg-7">
												<textarea class="form-control" rows="3" disabled="disabled">${ResultData.sql_where_clause}</textarea>
												</div>
											</div>
											
											<div class="form-group">
												<label class="col-lg-5 control-label" >GROUP块:</label>
												<div class="col-lg-7">
												<textarea class="form-control" rows="3" disabled="disabled">${ResultData.sql_group_clause}</textarea>
												</div>
											</div>
											
											<c:forEach var="entry" items="${ResultData.sort}" varStatus="status">
												<div class="form-group">
													<label class="col-lg-5 control-label">排序策略:</label>
													<div class="col-lg-7">${entry.key} : ${entry.value}</div>
												</div>
											</c:forEach>
											<hr />
											<div class="form-group">
												<div class="col-lg-offset-1 col-lg-9">
													<a class="btn btn-default btn-sm" href="/sql/edit?sql_id=${ResultData.id}">编辑</a>
													&nbsp;&nbsp;&nbsp;
													<a class="btn btn-default btn-sm" href="/report/create?sql_id=${ResultData.id}">使用</a>
												</div>
											</div>
										</c:if>
									</div>
